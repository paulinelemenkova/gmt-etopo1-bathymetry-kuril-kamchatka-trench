...
\cite{Nocquet2012,Kurt2023}. Building on these networks,
...
in tension \cite{McKenzie1972,Jackson1994,LePichonAngelier1979,Jolivet2013,Taymaz2007}. The combined
...
\cite{Aktug2009,SengorYilmaz1981,Bozkurt2001,TenVeen2009}. The most prominent of these are the Gediz
...
velocity \cite{McClusky2000,Kurt2023}. The vectors rotate from
...
towards the trench \cite{McKenzie1972,LePichonAngelier1979,Nocquet2012,Jolivet2013}.
...
$20$~mm\,yr$^{-1}$ \cite{Aktug2009,Taymaz2007}. The field is constrained by
...
localises along their margins \cite{McKenzie1972,LePichonAngelier1979,Jolivet2013}.
...
\subsection{GNSS velocity field}
\label{sec:gnss}

To characterise the present-day crustal kinematics of the Western Anatolian
Extensional Province we compiled GNSS station velocities from the Nevada
Geodetic Laboratory (NGL) global \texttt{MIDAS} solution
\citep{blewitt2016midas}, which provides robust, outlier-insensitive secular
velocities in the IGS14 (ITRF2014-aligned) reference frame. From the global
solution we extracted all continuous stations within the study window
(26--31\textdegree\,E, 36--40.5\textdegree\,N), yielding 50 sites, and within a
padded box (24--33\textdegree\,E, 34--42\textdegree\,N) used to constrain the
field margins, yielding 154 sites; reported $1\sigma$ horizontal uncertainties
are typically $0.2$--$0.8~\mathrm{mm\,yr^{-1}}$ for sites with $>5$\,yr of data.

To isolate intraplate deformation we rotated the velocities into a
Eurasia-fixed frame by removing the rigid rotation predicted by the ITRF2014
plate-motion model \citep{Altamimi2017}, whose Eurasia pole corresponds to an
angular velocity vector
\[
(\omega_x,\omega_y,\omega_z)
=
(-0.085,\,-0.531,\,+0.770)\ \mathrm{mas\,yr^{-1}},
\]
i.e.\ an Euler pole located at
\[
\Phi \approx 55.07^\circ\mathrm{N}, \qquad
\Lambda \approx 260.65^\circ\mathrm{E},
\]
with an angular rotation rate of
\[
\omega \approx 0.261^\circ\,\mathrm{Myr^{-1}}.
\]
An independent re-estimation using the ITRF2014 reference frame
\citep{Jagoda2020} reproduces these values closely, yielding
$\Phi = 54.81^\circ$, $\Lambda = 261.04^\circ$ and
$\omega = 0.2585^\circ\,\mathrm{Ma^{-1}}$. The reduction is validated by the
behaviour of stable-platform sites, which collapse to approximately zero
velocity (e.g.\ ZONG: $-0.1$, $-0.1~\mathrm{mm\,yr^{-1}}$, given as east and
north components), whereas Anatolian sites exhibit the expected
westward--southwestward motion relative to Eurasia (e.g.\ ANKR:
$23.5~\mathrm{mm\,yr^{-1}}$ westward; \.{I}ZM\.{I}R: $-22$,
$-17~\mathrm{mm\,yr^{-1}}$).

In the Eurasia-fixed frame the vectors are near-zero in the north (stable
Anatolia/Eurasia) and grow to $\mathrm{ca.}~25$--$30~\mathrm{mm\,yr^{-1}}$
toward the southwestern Aegean, rotating from W to SW and thereby illustrating
the characteristic Western Anatolian extension-and-extrusion pattern
(Fig.~\ref{fig:gnss_velocity}). This south-westward increase is consistent with
the regional kinematics of the Anatolian--Aegean domain
\citep{McClusky2000,briole2021aegean}, and the along-strike
velocity gradients across the major E--W grabens reflect active N--S extension,
in agreement with regional geodetic \citep{kurt2023turkey} and InSAR-derived
\citep{weiss2020anatolia} strain-rate estimates. All maps were produced with
the Generic Mapping Tools \citep{wessel2019gmt6}.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{fig_gnss_velocity.pdf}
  \caption{Eurasia-fixed GNSS horizontal velocities for the Western Anatolian
  Extensional Province, derived from the NGL \texttt{MIDAS} IGS14 solution
  \citep{blewitt2016midas} and reduced with the ITRF2014 plate-motion model
  \citep{Altamimi2017}. Arrows are horizontal velocities (scale vector
  $20~\mathrm{mm\,yr^{-1}}$, lower left); ellipses denote $1\sigma$
  uncertainties. Magenta lines are active faults and purple lines the graben
  axes. Shaded relief from GEBCO. Inset shows the regional setting.}
  \label{fig:gnss_velocity}
\end{figure}
...
\cite{Aktug2009,England2016,Walters2011,Hussain2016,Cakir2005,McNab2018,Floyd2010}. The continuum interpretation
